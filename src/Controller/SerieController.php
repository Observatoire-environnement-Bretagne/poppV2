...
class SerieController extends Controller
{
    ...
    public function viewSeries()
    {
        ...
        foreach($Series as $serie){
            ...
            if($serie->getSerieDepartement()){
                $tabDepartements[$serie->getSerieDepartement()->getDepartementId()] = $serie->getSerieDepartement()->getDepartementNom();
            }
    ...
    public function createSerie()
    {
        ...
        //Région par défaut
        //$regionDefault = $em->getRepository(Region::class)->findOneBy(array('regionNom' => self::DEFAULT_VALUE_SERIE['region']));
        //$serie->setSerieRegion($regionDefault);
        ...
        $regionDefault = $em->getRepository(Region::class)->findOneBy(array('regionNom' => $parameters["default_value_serie"]['region']));
        if ($regionDefault){
            $defaultValue['region'] = $regionDefault->getRegionId();
        }
        $paysFr = $em->getRepository(Pays::class)->findOneBy(array('paysNom' => $parameters["default_value_serie"]['pays']));
        if ($paysFr){
            $defaultValue['pays'] = $paysFr->getPaysId();
        }
        $langueFr = $em->getRepository(Langue::class)->findOneBy(array('langueNom' => $parameters["default_value_serie"]['langue']));
        if ($langueFr){
            $defaultValue['langue'] = $langueFr->getLangueId();
        }
        $defaultValue['frequence'] = $parameters["default_value_serie"]['frequence'];
        $evolutionDelete = $em->getRepository(EvolutionPaysage::class)->findOneBy(array('evolutionPaysageNom' => $parameters["default_value_serie"]['disparition']));
        if ($evolutionDelete){
            $defaultValue['evolution_delete'] = $evolutionDelete->getEvolutionPaysageId();
        }
        /*if ($langueFr && $evolutionDelete && $paysFr){
            $defaultValue = [
                'langue' => $langueFr->getLangueId(),
                'region' => $regionDefault->getRegionId(),
                'pays' => $paysFr->getPaysId(),
                'evolution_delete' => $evolutionDelete->getEvolutionPaysageId()
            ];
        }else{
            $defaultValue = [
                'langue' => '',
    ...
    { 
        ...
        foreach($tabDoc as $typeDoc => $infoDoc){
            ...
            if($infoDoc['fileId'] == 'new'){
                ...
                $newDoc->setFileManagerDate($datetime);
        ...
        foreach($request->request->get('photos') as $photoInfo){
            switch ($photoInfo['photo_action']) {
                ...
                case 'updated':
                    ...
                    $photoDatePrise = \DateTime::createFromFormat("j/m/Y", $photoInfo['photo_date_prise']);
                    ...
                    switch($photoInfo['photo_file_action']){
                        case 'new' :
                            ...
                            $newPhoto->setFileManagerDate($datetime);
            ...
            if($docRefAction == 'new' || $docRefAction == 'update'){
                ...
                if($request->request->get('serie_document_ref_file_action') != ""){
                    ...
                    if($refDocFile && $request->request->get('serie_document_ref_file_action') == 'loaded'){
                        ...
                        $docRefFileNew->setFileManagerDate($datetime);
                    ...
                    case 'updated':
                        ...
                        switch($sonInfo['son_file_action']){
                            case 'new' :
                                ...
                                $newSon->setFileManagerDate($datetime);
                    ...
                    case 'updated':
                        ...
                        switch($documentInfo['document_file_action']){
                            case 'new' :
                                ...
                                $newDocument->setFileManagerDate($datetime);
     ...
     * @return Response
     */
    public function updateSerie(string $serieId)
    {
        //Récupération du FileManagerDAO
        $fileManagerDAO = $this->get('filemanager.dao');
        //paramètres golobaux
