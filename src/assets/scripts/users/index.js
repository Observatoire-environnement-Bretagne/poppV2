...
export default (function () {
    ...
    $('#dataTableUsers tbody').on( 'click', 'a.remove', e => {
        if(confirm("Vous êtes sur le point de supprimer un utilisateur. Souhaitez vous continuer?")){
            var elem = e.currentTarget;
    ...
    var tableFournisseur = $('#dataTableFournisseurs').DataTable({
        ...
        columns: [  
            {data: "user_id"}, {data: "user_nom"}, {data: "user_prenom"}, {data: "user_email"},  
            {data: "user_sexe"}, {data: "user_date_dercnx"}, {data: "user_opp"}, {data: "action", width: "120px"}
        ],
        "columnDefs": [
            {
                "targets": [ 0, 6 ],
    ...
    $('#dataTableFournisseurs tbody').on( 'click', 'a.remove', e => {
        if(confirm("Vous êtes sur le point de supprimer un utilisateur. Souhaitez vous continuer?")){
            var elem = e.currentTarget;
            var ligneIndex = $(elem).parent().parent();
            var userId = tableFournisseur.row(ligneIndex).data().user_id;
            $.ajax({
                url: PARAMETRES.url + '/gestion/delete/user/' + userId,
            })
